---
search:
  exclude: true 
---

# Toowong Cemetery Explorer Card 

### Coming soon

The **Toowong Cemetery Explorer Card** is your digital companion for exploring the cemetery and staying connected with the Friends of Toowong Cemetery.

<!-- image -->

Add it to your Apple Wallet or Google Wallet to:

- Find graves and get directions
- Discover stories and interpretive displays nearby
- Receive updates about guided tours and volunteer events
- Stay connected with Friends of Toowong Cemetery news

Your card evolves as your involvement grows, from a Toowong Cemetery Explorer, to a Heritage Hero Volunteer, to a Friends of Toowong Cemetery Member.

### Get your free Toowong Cemetery Explorer Card

[![Add to Apple Wallet](../assets/add-to-apple-wallet.png){ width="150" }](#)

[![Add to Google Wallet](../assets/add-to-google-wallet.png){ width="150" }](#)

Scan the QR to add the Toowong Cemetery Explorer Card to your Apple or Google Wallet. 

[![Scan the QR code to add the card to your phone's wallet](../assets/qr-code-fotc-test-digital-card.png){ width="300" }](#)

